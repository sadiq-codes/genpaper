...
export function generateOutlineUserPrompt(
  ...
  // Paper IDs are NOT assigned in the outline. Retrieval handles evidence selection.
  // We only provide the count so the model can calibrate breadth without token blow-ups.
  const sourcesInfo = sourcePaperIds.length > 0
    ? `AVAILABLE SOURCE PAPERS: ${sourcePaperIds.length}
NOTE: You do NOT need to assign papers to sections. Evidence retrieval will select sources later.`
...
* candidatePaperIds: An array of paper IDs for this section. Since retrieval selects evidence later, you should set this to an empty array [] unless you have a strong reason to include specific IDs.
...
Your response must be valid JSON that matches this TypeScript type:
{
  sections: Array<{
    ...
    expectedWords: number;
    keyPoints: string[];
    candidatePaperIds: string[];  // Usually []
