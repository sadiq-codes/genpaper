-- ═══════════════════════════════════════════════════════════════════
-- COMPLETE SCHEMA REWRITE MIGRATION (SIMPLIFIED VERSION)
-- Reduces 18 tables to 8 tables while maintaining core functionality
-- Compatible with all SQL execution environments
-- ═══════════════════════════════════════════════════════════════════

-- 🚨 IMPORTANT: Make sure you have a full database backup before running!
-- 📋 This will reduce 18 tables to 8 tables (55% reduction)
...
-- 2.2 MIGRATE PROFILES FIRST (ensures FKs for projects/library)
INSERT INTO profiles_new (id, email, full_name, created_at)
SELECT id, email, full_name, created_at
FROM profiles
ON CONFLICT (id) DO NOTHING;

-- 2.3 MIGRATE PAPER CHUNKS
...
-- 2.5 MIGRATE CITATIONS (Combine project_citations and paper_references)
...
DROP VIEW IF EXISTS vector_search_performance CASCADE;
...
-- ═══════════════════════════════════════════════════════════════════
-- 
-- ✅ Schema reduced from 18 tables to 8 tables (55% reduction)
-- 🗑️  Backup tables created with _backup suffix
-- ⚠️   Test your application thoroughly before dropping backup tables
-- 🚀 Your database is now significantly simplified!
--
-- To verify migration success, run:
-- SELECT COUNT(*) FROM papers;
-- SELECT COUNT(*) FROM paper_chunks;
-- SELECT COUNT(*) FROM research_projects;
-- SELECT COUNT(*) FROM project_citations;
-- SELECT COUNT(*) FROM library_papers;
